...
for i = 1:populationSize/2
    ...
    %% arithmetic
    r_w=recombination_weight;
    offspring(2*i-1,:)=r_w*population(parentIndexes(1),:)+(1-r_w)*population(parentIndexes(2),:);
    offspring(2*i,:)=r_w*population(parentIndexes(2),:)+(1-r_w)*population(parentIndexes(1),:);
...
for i = 1:populationSize
    ...
    for j=1:dimension
        if rand()<mutation_rate
            offspring(i,j)=rand()*(upper_bound(j)-lower_bound(j))+lower_bound(j);
        end
    end
